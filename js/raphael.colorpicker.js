...
(function (Raphael) {
    ...
    Raphael.fn.colorPickerIcon = function (x, y, r) {
        ...
        for (var i = 0; i < segments; i++) {
           ...
           /* this.path(path).attr({
                stroke: "none",
                fill: "hsb(" + (segments - i) * (255 / segments ) / 355 + ", 1, 1)",
    ...
    var doc = document, win = window,
        ColorPicker = function (x, y, size, initcolor, element) {
            ...
            t.cpicon = r.colorPickerIcon(size2, size2, size2 - padding);

            t.cursor = r.set(); // outer rinf cursor
            //t.cursor.push(r.circle(111, 111, 10.1 ).attr({
            t.cursor.push(r.circle(140, 140, 13 ).attr({
                stroke: "#FFF",
                opacity: .9,
                "stroke-width": 2
            }));

             t.cursor.push(r.circle(90, 90, 8 ).attr({
                stroke: "#FFF",
                opacity: .5,
                "stroke-width": 8
            }));
//            console.log(t.cursor[0]);
//            console.log(t.cursor[0].clone);
             //inner ring
...
//            console.log(w1);
            ...
            t.disc.drag(function (dx, dy, x, y) {
                ...
                var ajcx = t.cursor[0].attrs.cx;
                var ajcy = t.cursor[0].attrs.cy;
                mappicker_ondrop(ajcx,ajcy,t);
    ...
    ColorPicker.prototype.setHS = function (x, y) {
        var X = x - this.size2,
            Y = y - this.size2,
            R = this.size2 - this.size20 / 2 - this.padding - 5, //the -5 shrinks the bounding circle.....(dnagerous though)
            d = angle(X, Y),
            rd = d * pi / 180;
        ...
        this.H = (1.05 - d / 360) % 1;   /// HERE IS WHERE WE WILL ADJ COLOR
        this.S = Math.min((X * X + Y * Y) / R / (R / 4) , 1); // Saturation /// CGS was ) / R / R , changed to  / R, 1
        this.onchange && this.onchange(this.color());
    };
